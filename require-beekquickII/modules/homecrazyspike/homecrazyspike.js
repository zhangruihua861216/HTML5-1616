define(["jquery","text!modules/homecrazyspike/homecrazyspike.html",
    "css!modules/homecrazyspike/homecrazyspike.css"],function($,html){
    function render(){
        $("main").empty();
        $("main").append(html);

        $.getJSON("http://www.vrserver.applinzi.com/aixianfeng/apimiaosha.php",function(data){
            if(data.product.length>0) {
                $.each(data.product, function (index, n) {
                    $(".best-seller").before(
                        $("<ul/>").attr("class","home-crazy-spike-product-part-1").append(
                            $("<li/>").append(
                                $("<img/>").attr({"src":n.img})
                            )
                        ).append(
                            $("<li/>").append(
                                ...
                                $("<div/>").append(
                                    ...
                                    $("<span/>").attr("class","oldPrice").html("/原价￥"+n.market_price+"元")
                                )

                            ).append(
                                    $("<div/>").attr("class","btnColor").append(
                                            $("<span/>").html(n.btnText)
                    ...
                    if(n.enable){
                        $(".btnColor span").css("background","red")
                    }
        ...
        function  squareFun(kk){
            ...
            switch (kk){
                case 0:
                    if(hours<=9){
                        var Diffhours=getTwoNum(9-hours);
